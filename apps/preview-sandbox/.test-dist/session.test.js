...
test('createPreviewSession creates new session status', () => {
    ...
    const session = createPreviewSession({ tenantId: 'tenant-a', principalSub: 'user-a' }, 'product-a', 'pilot-adapter', '1.0.0', now);
...
test('transitionPreviewSession updates status and timestamp', () => {
    const initial = createPreviewSession({ tenantId: 'tenant-a', principalSub: 'user-a' }, 'product-a', 'pilot-adapter', '1.0.0', new Date('2026-03-10T00:00:00.000Z'));
