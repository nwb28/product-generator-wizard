# Documentation Index

Documentation is organized by domain so operational and governance assets stay discoverable as the project scales.

## API
- `docs/api/openapi.yaml`

## Reference
- `docs/reference/user-guide.md`
- `docs/reference/maintainer-guide.md`
- `docs/reference/environment-config.md`
- `docs/reference/api-client.md`
- `docs/reference/roadmap.md`
- `docs/reference/phase7-architecture-rfc.md`
- `docs/reference/adapter-authoring-guide.md`
- `docs/reference/preview-operator-guide.md`
- `docs/reference/adr/0001-build-artifact-strategy.md`

## Operations
- `docs/operations/deployment-workflow.md`
- `docs/operations/post-release-verification.md`
- `docs/operations/rollout-strategy.md`
- `docs/operations/disaster-recovery-runbook.md`
- `docs/operations/redis-resilience.md`
- `docs/operations/slo-observability.md`
- `docs/operations/slo-alert-runbook.md`
- `docs/operations/alert-routing.md`
- `docs/operations/performance-baseline.md`
- `docs/operations/operations-log.md`
- `docs/operations/audit-forwarder.md`
- `docs/operations/preview-runbook.md`
- `docs/operations/governance-bootstrap-runbook.md`

## Governance
- `docs/governance/contract-governance.md`
- `docs/governance/compliance-package.md`
- `docs/governance/tenant-quotas.md`
- `docs/governance/tenant-quota-change-workflow.md`
- `docs/governance/vulnerability-gate-policy.md`
- `docs/governance/remediation-tracker.md`
- `docs/governance/secret-rotation-evidence.json`
- `docs/governance/sbom-process.md`
- `docs/governance/adapter-versioning-policy.md`
- `docs/governance/pre-inclusion-policy.md`
- `docs/governance/preview-threat-model.md`
- `docs/governance/soc2-control-matrix.md`

## Reports
- `docs/reports/pilot-execution-report.md`
- `docs/reports/pilot-preview-execution-report.md`
